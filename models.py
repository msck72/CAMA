...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1, rate = 1, track=False):
        ...
        self.bn1 = nn.BatchNorm2d(planes, track_running_stats = track) if track is False else nn.BatchNorm2d(planes)
        self.conv2 = nn.Conv2d(planes, planes, kernel_size=3, stride=1, padding=1, bias=False)
        self.bn2 = nn.BatchNorm2d(planes, track_running_stats = track) if track is False else nn.BatchNorm2d(planes)
...
class ResNet(nn.Module):
    def __init__(self, block, hidden_layers , num_blocks, num_classes=10, model_rate=1, rate=1 , track = False):
        ...
        self.bn1 = nn.BatchNorm2d(hidden_layers[0], track_running_stats = track)
